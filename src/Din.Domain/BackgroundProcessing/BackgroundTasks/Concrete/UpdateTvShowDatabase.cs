...
namespace Din.Domain.BackgroundProcessing.BackgroundTasks.Concrete
{
    public class UpdateTvShowDatabase : BackgroundTask
    {
        ...
        ) : base(container, logger, nameof(UpdateTvShowDatabase))
        {
        ...
        protected override async Task OnExecuteAsync(Scope scope, CancellationToken cancellationToken)
        {
            ...
            var episodesToAdd = (await GetEpisodesToAddAsync(scope, repository, storedTvShows, cancellationToken))
                .ToList();
        ...
        private static async Task<ICollection<SonarrTvShow>> GetExternalTvShowsAsync(Scope scope,
        ...
        private static async Task CheckGenresAsync(Scope scope, IEnumerable<SonarrTvShow> externalTvShows,
            CancellationToken cancellationToken)
        {
        ...
        private IEnumerable<TvShow> GetTvShowsToAdd(Scope scope, ITvShowRepository repository,
            IEnumerable<SonarrTvShow> externalTvShows, ICollection<TvShow> storedTvShows)
        {
            ...
            foreach (var tvShow in externalTvShows)
            {
                ...
                {
                    repository.Update(storedTvShow.Update(tvShow));
                }
            }
        }

        private async Task<IEnumerable<Episode>> GetEpisodesToAddAsync(Scope scope, ITvShowRepository repository,
            IEnumerable<TvShow> tvShows, CancellationToken cancellationToken)
        {
            var sonarrClient = scope.GetInstance<ISonarrClient>();
            ...
            foreach (var tvShow in tvShows)
            {
                ...
                Parallel.ForEach(externalEpisodes, externalEpisode =>
                {
                    ...
                    {
                        var episode = externalEpisode.ToEntity();
        ...
        private Episode? GetStoredEpisode(IReadOnlyCollection<Episode> storedEpisodes, SonarrEpisode externalEpisode,
            TvShow tvShow)
        {
